  ...
  MessageCreate m -> unless (fromBot m)
    $ do
      let mt = fromStrict $ messageText m
          ps =
            runParser (ws $ (string "🤠" <|> string "boy howdy") >> word) mt
      interpret (maybe "" fst ps) (m, maybe "" snd ps)
        $ do
          command "about" about
          command "bless" bless
          ...
          command "uwu" uwu
  MessageReactionAdd
    r -> interpret (fromStrict $ emojiName $ reactionEmoji r) r
    $ do
      reaction "♋" uwuR
      reaction "🔣"
        $ translate English =<< asksExposed (fromStrict . messageText)
      reaction "🗺️"
        $ do
