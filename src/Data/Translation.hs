...
translate :: forall a m. (MonadIO m, MonadThrow m, MonadFail m) => LocalizedText a -> Lang -> m (LocalizedText KLang)
translate t' tl = do
    let t  = locText t'
        fl = whatLang' t'
    ...
    let resBody = getResponseBody @APIResponse response
        resText = toLazyText $ htmlEncodedText $ translation resBody
        detLang = fl <|> detectedLang resBody

    when (isNothing detLang) $ fail "Couldn't detect language"
